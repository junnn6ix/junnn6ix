<!-- <img src="https://github.com/reeveng/reeveng/blob/master/svg.svg"/> -->

<p><em>An Informatics Engineer student att <a href="https://sttcipasung.ac.id/">STT Cipasung</a>, interested in many things such a Web Programming, UI/UX etc</br>
</em></p>

```javascript
const aboutJun = {
   pronouns: "he" | "him",
   studied: ["Javascript", "HTML", "CSS"],
   onStudying: ["Typescript", "Kotlin", "PHP"],
   technologies: {
      frontEnd: {
         js: ["React"],
         css: ["Tailwind"]
      },
      backEnd: {
         js: [""]
      },
      databases: [""]
   },
   socials: {
      ig: "https://www.instagram.com/neckjun__/",
      linkedin: "https://www.linkedin.com/in/neckjss/",
      github: "you're on my github rn"
   },
   currentOccupation: ["Work at Unemployed Guy, open for job opportunities"],
   challenge: "I'm working towards being able to doing nothing."
};
```

<!-- </br></br>
<h2>💻 Some stats 💻</h2>

![Reeveng's github stats](https://github-readme-stats.vercel.app/api?username=reeveng&show_icons=true&title_color=fff&icon_color=79ff97&text_color=9f9f9f&bg_color=151515)

---

⭐️ From [reeveng](https://github.com/reeveng) inspired by [sudiptog81](https://github.com/sudiptog81) and  [FelixRilling](https://github.com/) -->
<!--backEnd: {
         java: ["Spring"],
         js: ["Node", "Express"],
         csharp: ["Asp.net Core"],
         misc: ["Bash", "Puppeteer", "Selenium", "Cypress"]
      },
      databases: ["MongoDB", "mySQL", "SQLServer"],
      mobile: ["Android", "IOS"] -->
